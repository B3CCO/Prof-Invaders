...
public class ProfInvaders extends Application {
	...
	public void start(Stage primaryStage) throws Exception {
		...
		// Mantenimento del focus per gestire input da tastiera
	...
	private void TastoPremuto(KeyEvent tasto) {
		// Movimento a destra/sinistra o sparo
	...
	private void MuoviColpi() {
		// movimento e collisione dei colpi con gli invasori
	...
	private void MuoviInvasori() {// Muovi invasori e controllo fine ondata e vittoria
		if (ondateVinte == 0) {
		...
		// Movimento orizzontale degli invasori
		...
		// discesa se invasori arrivano ai bordi
		if (massimo >= 515) {
		...
		// Controllo se gli invasori raggiungono il giocatore
	...
	private void resetInvasori() { // Posizionamento iniziale degli invasori per ogni ondata
		for (ImageView invasore : invasori) {
			sfondo.getChildren().remove(invasore);
		}
